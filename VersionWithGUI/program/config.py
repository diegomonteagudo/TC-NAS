...
def produce(program_path, json_path):
    json_object = open_file(json_path)
    import shutil

    config_results_path = program_path + "/config_results/"
    if os.path.exists(config_results_path):
        shutil.rmtree(config_results_path)
    os.mkdir(config_results_path)

    for r in json_object['routers'] :
    
        #ecrire dans le fichier json
        destination=config_results_path+r['hostname']+".cfg"
        with open(destination, "w") as outfile:
            outfile.write(write_file(r))